...
Today we will perform an RNASeq analysis on the effects of dexamethasone (hereafter "dex"), a comon steroid, on airway smooth muscle (ASM) cell lines.
...
The main function in this package is called `DESeq()` it will run the full analysis for us on our `dds` unput object:
...
A useful summary figure of our results is often called a volcano plot. It is basicaly a plot of log2 fold change values vs Adjusted P-values.

> Q. Use ggplot to make a first version "volcanoplot" of `log2FoldChange` vs ``padj`
...
This is not very useful becasue the y-axis (P-value) is not really helpful - we want to focus on low P-values.
...
> Q. Add color to the points (genes) we care about, nice axis labels, useful title and a nice theme.
...
To make sense of our results we need to know what the differential expressed genes are and what biological pathways and process they are involved in.
...
To run our pathway analysis we will use the **gage()** function. It wants to two main inputs: a vector of importance (in our case the log2 fold change values); and the gene sets to check overlap for.
...
> Q. Generate and insert a path way figure for "Graft-versus-host disease" and "Type I diabetes"?
